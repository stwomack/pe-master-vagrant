...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.define "puppetmaster", primary: true do |puppetmaster| 
    ...
    # Configure r10k
    puppetmaster.vm.provision :shell, :inline => "sudo git clone git@prodgithub01.wwt.com:Operations/wwt-pe-master-configuration.git /etc/puppetlabs/puppet/wwt-pe-master-configuration"
    puppetmaster.vm.provision :shell, :inline => "sudo ln -sf /etc/puppetlabs/puppet/wwt-pe-master-configuration/hiera.yaml /etc/puppetlabs/puppet/hiera.yaml"
    puppetmaster.vm.provision :shell, :inline => "sudo ln -sf /etc/puppetlabs/puppet/wwt-pe-master-configuration/r10k.yaml /etc/puppetlabs/puppet/r10k.yaml"
    puppetmaster.vm.provision :shell, :inline => "sudo r10k deploy environment sand -p -v -c /etc/puppetlabs/puppet/r10k.yaml"
    puppetmaster.vm.provision :shell, :inline => "sudo /etc/puppetlabs/puppet/wwt-pe-master-configuration/vagrant_create_classifiers.sh"
  end

  (1..4).each do |i|
     config.vm.define "sandmongodb0#{i}" do |node|
       node.vm.box = "centos-6.4-x86_64"
       node.vm.hostname = "sandmongodb0#{i}.wwt.com"
       ...
       node.vm.provision :shell, :inline => "sudo echo '10.10.10.101 sandmongodb01.wwt.com' >> /etc/hosts"
       node.vm.provision :shell, :inline => "sudo echo '10.10.10.102 sandmongodb02.wwt.com' >> /etc/hosts"
       node.vm.provision :shell, :inline => "sudo echo '10.10.10.103 sandmongodb03.wwt.com' >> /etc/hosts"
       node.vm.provision :shell, :inline => "sudo echo '10.10.10.104 sandmongodb04.wwt.com' >> /etc/hosts"
       node.vm.provision :shell, :inline => "sudo echo '10.10.10.105 sandmongodbarb01.wwt.com' >> /etc/hosts"
  ...
  config.vm.define "sandmongodbarb01" do |node|
    node.vm.box = "centos-6.4-x86_64"
    node.vm.hostname = "sandmongodbarb01.wwt.com"
    node.vm.box_url = "http://developer.nrel.gov/downloads/vagrant-boxes/CentOS-6.4-x86_64-v20131103.box"
    node.vm.box_download_insecure = true
